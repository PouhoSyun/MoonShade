...
- 校内邮箱注册与密码重置：支持 10 位数字 + `@pku.edu.cn`、10 位数字 + `@stu.pku.edu.cn`，以及 10 位数字 + `@ruc.edu.cn`，可通过邮件验证码重置密码。
- 问卷系统：按卷填写，必填项不完整时提示需要更新的卷序号，兴趣爱好卷为选填。
- 暂停匹配：用户可在状态页暂停或恢复匹配；暂停期间保留问卷和历史结果，但不进入新候选。
- 用户端匹配结果：只有管理员发布后的匹配会展示给用户，并附带对方资料、共同兴趣和联系方式。
- 微信交流群：首页可打开二维码弹层，二维码由管理员后台上传和删除。
- 管理员后台：查看已提交问卷、管理公告、管理微信群二维码、生成测试用户、查看候选匹配、推送匹配、撤回已发布匹配。
...
生产环境必须设置 `MOONSHADE_ADMIN_PASSWORD`，否则服务会拒绝启动，避免线上误用默认管理员密码。开发模式可以继续使用默认管理员账号。
...
正式部署时推荐使用 Brevo SMTP Relay。生产环境没有配置 SMTP/sendmail 时不会把验证码返回到页面；本地开发如需显示验证码，需要设置 `NODE_ENV=development` 或 `MOONSHADE_ALLOW_DEV_CODE=1`。
...
- 基础身份：展示名、性别、希望匹配性别、出生年、身份、院校背景、校区。
...
用户暂停匹配后，管理员已提交问卷列表会把该用户灰色列在暂停区；暂停用户不参与权重计算、自动候选生成，也不会出现在管理员手动调整候选的下拉选择中。恢复匹配后会重新进入候选池。

公告可以在管理员后台新增、修改、删除。用户首页会读取当前公告列表展示。

微信群二维码可以在管理员后台上传、替换或删除。上传后的二维码会显示在首页“微信群”入口中。
...
- `rawPrecisionRatio(profile)`：按可接受范围的宽窄计算原始精准度，多选越宽，原始精准度越低。
...
- `precisionCoefficient` 先按关键“可接受”多选范围计算原始精准度，再在当前画像池中按排名重映射到约 `0.7-1.3`。多选越宽，原始精准度越低。
...
- `scarcityCoefficient` 先按身份、家乡、成长环境、专业、关系期待等字段计算原始稀缺度，再在当前画像池中按排名重映射到约 `0.7-1.3`。
...
正式长期运行前建议继续补充数据导出、管理员操作日志、更细的隐私开关和自动备份。
